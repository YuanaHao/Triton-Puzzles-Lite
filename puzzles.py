...
def add_mask2_kernel(x_ptr, z_ptr, N0, B0: tl.constexpr):
    ...
    off_x = off_start + tl.arange(0, B0)
    # create a mask to prevent out of bounds access
    mask_range = off_x < N0
    # load data
    x = tl.load(x_ptr + off_x, mask=mask_range, other=0)
    # execute calculation
    z = add2_spec(x)
    # store result
    tl.store(z_ptr + off_x, z, mask=mask_range)
...
def add_vec_kernel(x_ptr, y_ptr, z_ptr, N0, N1, B0: tl.constexpr, B1: tl.constexpr):
    ...
    z = add_vec_spec(x, y)
    off_i = tl.arange(0, B0)[None, :]  # cols
    off_j = tl.arange(0, B1)[:, None]  # rows
    offset = off_j * N0 + off_i
